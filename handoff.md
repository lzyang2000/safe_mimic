...
- BALLET LIBRARY TRIMMED 2026-09-09: `DEFAULT_G1_BALLET_MANIFEST` -> `artifacts/bones-seed/datasets/g1_ballet_v1_trim1s/ballet.yaml` (at most ~1 s of standing stance kept at each end of every clip; `scripts/trim_motion_stance.py`, 2212 s -> 1942 s, 196/348 clips touched). All ballet checkpoints on record trained on the UNTRIMMED `g1_ballet_v1/ballet.yaml`; pass that path as `--motion-file` to reproduce their numbers.
...
23. **Escape moves (branch `escape-moves`, 2026-09-10, built, NOT trained).** Ballet steps as the avoidance response: a sustained planar CBF correction switches the RAW clip to an indexed travelling move aligned with the escape direction (mirrored library `g1_ballet_v1_trim1s_mirror`, 696 clips, `escape_moves.json` index), blends in over 0.3 s, resumes the interrupted clip at its saved frame; CBF still on top; deployment trigger is the head's planar prediction. Task `...-CoAdjust-Unified-Joint-Leash-Ballet-Moves`, variant `coadjust-unified-joint-leash-ballet-moves`, default off everywhere else. Spec/plan under `docs/superpowers/`. Train: `uv run train SafeMimic-Tracking-LiveAligned-Crowd-Human-Unitree-G1-Lidar-Auxiliary-CoAdjust-Unified-Joint-Leash-Ballet-Moves --env.scene.num-envs 4096 --agent.run-name coadjust_unified_joint_leash_ballet_moves_scratch_4096`; gate on the slow preset vs ballet@30k plus `fair_regime.escape_resolved_rate`. RESULT (2026-09-11, 30k, slow preset, safety/tracking/survival %): moves ON on the mirrored library 2.1 / 20.3 / 77.5 (escape_resolved 0 %, 1.55 moves/episode); moves OFF on the dance clip 7.3 / 8.3 / 84.4; canonical ballet@30k 2.9 / 2.1 / 95.0 dance, 2.2 / 13.8 / 84.0 ballet ref. Moves cut safety failures 3.5x vs the same checkpoint without them but never resolved an encounter alone (CBF still intervening ~0.7 m/s during moves; planner active ~49 % of training steps at the 0.4 m/s trigger) and wrist trips rose to 20 %. NOT adopted. Next knobs (unapplied): trigger 0.6-0.8 m/s or walking-human-only trigger; ee_body_pos held off during blends / longer blend; per-switch diagnostic log. Checkpoint `logs/rsl_rl/*leash_ballet_moves/2026-09-10_14-29-09_*/model_29999.pt`.
